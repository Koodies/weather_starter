...
export function HourlyStrip({ periods = [] }: HourlyStripProps) {
  if (periods.length === 0) {
    return (
      <section className="panel border-[length:var(--border-w)] border-[rgb(var(--ink)/15%)] bg-[rgb(var(--ink)/8%)] backdrop-blur-xl">
        ...
        <div className="flex min-h-[5rem] items-center justify-center text-sm text-[rgb(var(--ink)/55%)]">
          --
        </div>
      </section>
    );
  ...
  return (
    <section className="rounded-2xl border border-[rgb(var(--ink)/15%)] bg-[rgb(var(--ink)/8%)] backdrop-blur-xl">
      ...
      <div
        className="grid divide-x divide-[rgb(var(--ink)/5%)]"
        ...
        {slots.map((slot) => {
          ...
          return (
            <div key={slot.key} className="flex flex-col items-center gap-2 px-2 py-4 text-center">
              <div className="text-xs font-medium text-[rgb(var(--ink)/85%)]">{slot.label}</div>
              {isFair ? (
                <SunIcon className="h-7 w-7 text-amber-300" />
              ) : (
                <CloudIcon className="h-7 w-7 text-[rgb(var(--ink)/85%)]" />
              )}
              <div className="text-xs leading-snug text-[rgb(var(--ink)/90%)]">{slot.forecast}</div>
            </div>
          );
        })}
      </div>
