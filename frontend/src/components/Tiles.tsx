...
function TileShell({ icon, title, className = '', children }: TileShellProps) {
  return (
    <section
      className={`panel flex flex-col gap-3 border-[length:var(--border-w)] border-[rgb(var(--ink)/15%)] bg-[rgb(var(--ink)/8%)] p-4 backdrop-blur-xl ${className}`}
    >
...
function ScaleBar({ value, max, gradientClass }: ScaleBarProps) {
  ...
  return (
    <div className="relative mt-3 h-1.5">
      ...
      {hasValue && (
        <div
          className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[rgb(var(--ink))] bg-[rgb(var(--ink))] shadow shadow-[rgb(var(--recede)/40%)]"
          style={{ left: `${pct}%` }}
        />
      )}
    </div>
...
export function AirQualityTile({ weather }: WeatherProps) {
  ...
  return (
    ...
    >
      <div className="text-4xl font-light leading-none text-[rgb(var(--ink)/95%)]">{psi}</div>
      <div className="mt-1 text-base text-[rgb(var(--ink)/85%)]">
      ...
      />
      <p className="mt-3 text-xs leading-snug text-[rgb(var(--ink)/70%)]">
    ...
    <TileShell icon={<WindIcon />} title="Wind" className="col-span-2">
      <div className="grid grid-cols-[1fr_auto] items-center gap-4">
        <ul className="space-y-2 text-sm">
          <li
            className={`flex justify-between ${hasDirection ? 'border-b border-[rgb(var(--ink)/10%)] pb-2' : ''}`}
          >
            <span className="text-[rgb(var(--ink)/75%)]">Wind</span>
            <span className="tabular-nums text-[rgb(var(--ink)/90%)]">{speed} km/h</span>
          </li>
          {hasDirection && (
            <li className="flex justify-between">
              <span className="text-[rgb(var(--ink)/75%)]">Direction</span>
              <span className="tabular-nums text-[rgb(var(--ink)/90%)]">{direction}&deg;</span>
            </li>
          )}
        </ul>
...
function Compass({ speed, direction }: CompassProps) {
  ...
  return (
    <div className="relative h-20 w-20 rounded-full border border-[rgb(var(--ink)/20%)] bg-[rgb(var(--ink)/4%)]">
      <span className="absolute left-1/2 top-1 -translate-x-1/2 text-[10px] text-[rgb(var(--ink)/55%)]">
        N
      </span>
      <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[10px] text-[rgb(var(--ink)/55%)]">
        E
      </span>
      <span className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[10px] text-[rgb(var(--ink)/55%)]">
        S
      </span>
      <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[10px] text-[rgb(var(--ink)/55%)]">
        W
      </span>
      {hasDirection && (
        <div
          className="absolute left-1/2 top-2 h-7 w-px origin-[50%_30px] -translate-x-1/2 rounded-full bg-[rgb(var(--ink)/70%)]"
          style={{ transform: `translateX(-50%) rotate(${direction}deg)` }}
        />
      )}
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-base font-light leading-none text-[rgb(var(--ink)/90%)]">
          {speed}
        </span>
        <span className="text-[10px] text-[rgb(var(--ink)/55%)]">km/h</span>
      </div>
    </div>
  );
}
...
export function UVTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<SunIcon className="h-3.5 w-3.5" />} title="UV Index">
      <div className="text-4xl font-light leading-none text-[rgb(var(--ink)/95%)]">
        {formatNumber(weather?.uv_index)}
      </div>
      <div className="mt-1 text-base text-[rgb(var(--ink)/85%)]">{uvLabel(weather?.uv_index)}</div>
      <ScaleBar
...
export function TemperatureTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<ThermometerIcon />} title="Temperature">
      <div className="text-4xl font-light leading-none tabular-nums text-[rgb(var(--ink)/95%)]">
...
export function PrecipitationTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<DropletIcon />} title="Rainfall">
      ...
      <div className="mt-1 text-sm text-[rgb(var(--ink)/85%)]">Latest reading</div>
      <p className="mt-3 text-xs leading-snug text-[rgb(var(--ink)/70%)]">
...
export function AveragesTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<TrendIcon />} title="Forecast High">
      ...
      <div className="mt-1 text-xs leading-snug text-[rgb(var(--ink)/75%)]">
        Today&apos;s high from the 24-hour forecast.
      </div>
      <ul className="mt-3 space-y-1 text-xs text-[rgb(var(--ink)/70%)]">
        <li className="flex justify-between border-t border-[rgb(var(--ink)/10%)] pt-1.5">
          <span>Today</span>
